...
def preprocessing():
    ...
    X = df.drop(['class'], axis=1)                              # fixed axis argument
    X = (X - X.min()) / (X.max() - X.min())                     # standardize data
    X = X.replace(np.NaN, 0)
    y = df['class'].transform(lambda x: 1 if x == 'g' else 0)   # fixed string comparison
    return X, y
...
def classification(X, y, learning_rate, num_epochs, verbose):
    ...
    if verbose:
        ...
        # Assuming compute_accuracy returns percentage (0-100)
